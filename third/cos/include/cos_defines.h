...
struct Content {
  ...
  std::string m_last_modified;           // Object ???????
  std::string m_etag;                    // ??? MD-5 ?????
  std::string m_size;                    // ???????? Byte
  std::vector<std::string> m_owner_ids;  // Bucket ?????
  std::string
      m_storage_class;  // Object ??????????STANDARD?STANDARD_IA
};
...
struct ReplicationRule {
  ...
  std::string m_dest_storage_class;  // ???
...
// ???? Bucket ???
struct Bucket {
  std::string m_name;         // Bucket ??
  std::string m_location;     // Bucket ????
  std::string m_create_date;  // Bucket ?????ISO8601 ?????
...
struct LiveRecord {
  ...
  std::string m_remote_addr;  // ???ip?????
  std::string m_request_id;   // ??ID
};
...
struct ListLiveChannelResult {
  std::string
      m_max_keys;  // ?????????????????????? max-keys ??
  std::string m_marker;  // ??????????????????? UTF-8
                         // ??????????????marker??
  std::string m_prefix;  // ????????????????????????
  std::string m_is_truncated;  // ???????????????? true ? false
  std::string
      m_next_marker;  // ??????????IsTruncated ?
                      // true?????????
                      // ???????????????????????????????????
                      // ?????????????? marker ????
